...
- Sort by Corfu history outside the minibuffer when ~corfu-history-mode~ is
  active.
- Highlight matches with standard completion faces, and highlight candidates
  whose search terms match word prefixes as a whole with ~flex-x-highlight~.
...
space-separated search term matches at the start of the candidate or after a
separator such as ~-~, ~_~, ~/~, ~.~, or whitespace. Thus ~agent~ highlights
both ~agent-shell~ and ~gnus-agentize~, and ~sw bu~ highlights
~switch-to-buffer~. Non-contiguous flex matches such as ~agt~ in
