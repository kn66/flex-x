...
filtering after a space, match highlighting, and optional extra matching for
...
Before a space is entered, the input uses built-in flex matching.  After a
space is entered, every non-empty term must match as a literal substring or by
one of the configured extra matchers.  Terms may occur in any order.
